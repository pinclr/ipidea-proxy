...
class TestApi(object):
    ...
    def split_whitelist_ips(self, white_ips):
        if ',' in white_ips:
    ...
    def test_whitelists_were_added(self, white_ips):
        f = self.get_full_whitelisted_ips()
        s = self.split_whitelist_ips(white_ips)
        assert set(s) < set(f)
    
    
    def test_delete_whitelist_msg_equals_success(self, white_ips):
        assert self.ipp.delete_whitelist(white_ips)['msg'] == 'success'
        
    
    def test_ips_are_not_whitelisted(self, white_ips):
        f = self.get_full_whitelisted_ips()
        s = self.split_whitelist_ips(white_ips) 
